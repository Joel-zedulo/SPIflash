...
// EDIT: Commented out a block holding a logic fragment (Quartus issue: multiple drivers)
/*
...
// EDIT: Wrapped a troublesome non-synthesize-able block with for conditional compilation(cond: in simulation)
`ifdef SIM
...
// EDIT: Eliminated multiple driver logic

///////////////////////////////////////////////

    ////    ////   ////    ////   ////
//    BUFFER  STREAM  MERGER  APPROACH (works abit like: demultiplexing meets C Pointers)
       ////    ////   ////    ////   ////

///////////////////////////////////////////////

// EDIT: Replacing conflicting 'driver' blocks with versions where buffers have been cloned
//        From quartus synthesis error logs i know we need to make streams for:
//        `mem`,
//        `qo_reg`,
//        `ntf_tcs`,
//        `ntf_tas`,
...
reg	[7:0]	mem_stream1 [511:0];
reg	[7:0]	mem_stream2 [511:0];

reg	[7:0]	qo_reg_stream1;
reg	[7:0]	qo_reg_stream2;
...
reg       ntf_twis_stream3;
reg       ntf_twis_stream4;
...
reg       ntf_twih_stream2;
reg       ntf_twih_stream3;
reg       ntf_twih_stream4;

reg mem_stream1_updated = 1'b0;
reg mem_stream2_updated = 1'b0;

reg qo_reg_stream1_updated = 1'b0;
reg qo_reg_stream2_updated = 1'b0;

reg ntf_tcs_stream3_updated = 1'b0;
reg ntf_tcs_stream4_updated = 1'b0;

reg ntf_tas_stream3_updated = 1'b0;
reg ntf_tas_stream4_updated = 1'b0;

reg ntf_tds_stream3_updated = 1'b0;
reg ntf_tds_stream4_updated = 1'b0;

reg ntf_tws_stream3_updated = 1'b0;
reg ntf_tws_stream4_updated = 1'b0;

reg ntf_twis_stream3_updated = 1'b0;
reg ntf_twis_stream4_updated = 1'b0;

reg ntf_tch_stream2_updated = 1'b0;
reg ntf_tch_stream3_updated = 1'b0;
reg ntf_tch_stream4_updated = 1'b0;

reg ntf_tah_stream2_updated = 1'b0;
reg ntf_tah_stream3_updated = 1'b0;
reg ntf_tah_stream4_updated = 1'b0;

reg ntf_tdh_stream2_updated = 1'b0;
reg ntf_tdh_stream3_updated = 1'b0;
reg ntf_tdh_stream4_updated = 1'b0;

reg ntf_twh_stream2_updated = 1'b0;
reg ntf_twh_stream3_updated = 1'b0;
reg ntf_twh_stream4_updated = 1'b0;

reg ntf_twih_stream2_updated = 1'b0;
reg ntf_twih_stream3_updated = 1'b0;
reg ntf_twih_stream4_updated = 1'b0;

wire any_mem_stream_updated;
wire any_qo_reg_stream_updated;
wire any_ntf_tcs_stream_updated;
wire any_ntf_tas_stream_updated;
wire any_ntf_tds_stream_updated;
wire any_ntf_tws_stream_updated;
wire any_ntf_twis_stream_updated;
wire any_ntf_tch_stream_updated;
wire any_ntf_tah_stream_updated;
wire any_ntf_tdh_stream_updated;
wire any_ntf_twh_stream_updated;
wire any_ntf_twih_stream_updated;

assign any_mem_stream_updated = |{
  mem_stream1_updated,
  mem_stream2_updated
};

assign any_qo_reg_stream_updated = |{
  qo_reg_stream1_updated,
  qo_reg_stream2_updated
};

assign any_ntf_tcs_stream_updated = |{
  ntf_tcs_stream3_updated,
  ntf_tcs_stream4_updated
};

assign any_ntf_tas_stream_updated = |{
  ntf_tas_stream3_updated,
  ntf_tas_stream4_updated
};

assign any_ntf_tds_stream_updated = |{
  ntf_tds_stream3_updated,
  ntf_tds_stream4_updated
};

assign any_ntf_tws_stream_updated = |{
  ntf_tws_stream3_updated,
  ntf_tws_stream4_updated
};

assign any_ntf_twis_stream_updated = |{
  ntf_twis_stream3_updated,
  ntf_twis_stream4_updated
};

assign any_ntf_tch_stream_updated = |{
  ntf_tch_stream2_updated,
  ntf_tch_stream3_updated,
  ntf_tch_stream4_updated
};

assign any_ntf_tah_stream_updated = |{
  ntf_tah_stream2_updated,
  ntf_tah_stream3_updated,
  ntf_tah_stream4_updated
};

assign any_ntf_tdh_stream_updated = |{
  ntf_tdh_stream2_updated,
  ntf_tdh_stream3_updated,
  ntf_tdh_stream4_updated
};

assign any_ntf_twh_stream_updated = |{
  ntf_twh_stream2_updated,
  ntf_twh_stream3_updated,
  ntf_twh_stream4_updated
};

assign any_ntf_twih_stream_updated = |{
  ntf_twih_stream2_updated,
  ntf_twih_stream3_updated,
  ntf_twih_stream4_updated
};

reg [9:0] mem_index;

always @(posedge any_mem_stream_updated) begin
  if (mem_stream1_updated) begin
    for (mem_index= 0; mem_index < 512; mem_index=mem_index +1)
      mem[mem_index] = mem_stream1[mem_index];
  end else if (mem_stream2_updated) begin
    for (mem_index= 0; mem_index < 512; mem_index=mem_index +1)
      mem[mem_index] = mem_stream2[mem_index];
  end
end

always @(posedge any_qo_reg_stream_updated) begin
  if (qo_reg_stream1_updated) begin
      qo_reg = qo_reg_stream1;
  end else if (qo_reg_stream2_updated) begin
      qo_reg = qo_reg_stream2;
  end
end

always @(posedge any_ntf_tcs_stream_updated) begin
  if (ntf_tcs_stream3_updated) begin
      ntf_tcs = ntf_tcs_stream3;
  end else if (ntf_tcs_stream4_updated) begin
      ntf_tcs = ntf_tcs_stream4;
  end
end

always @(posedge any_ntf_tas_stream_updated) begin
  if (ntf_tas_stream3_updated) begin
      ntf_tas = ntf_tas_stream3;
  end else if (ntf_tas_stream4_updated) begin
      ntf_tas = ntf_tas_stream4;
  end
end

always @(posedge any_ntf_tds_stream_updated) begin
  if (ntf_tds_stream3_updated) begin
      ntf_tds = ntf_tds_stream3;
  end else if (ntf_tds_stream4_updated) begin
      ntf_tds = ntf_tds_stream4;
  end
end

always @(posedge any_ntf_tws_stream_updated) begin
  if (ntf_tws_stream3_updated) begin
      ntf_tws = ntf_tws_stream3;
  end else if (ntf_tws_stream4_updated) begin
      ntf_tws = ntf_tws_stream4;
  end
end

always @(posedge any_ntf_twis_stream_updated) begin
  if (ntf_twis_stream3_updated) begin
      ntf_twis = ntf_twis_stream3;
  end else if (ntf_twis_stream4_updated) begin
      ntf_twis = ntf_twis_stream4;
  end
end

always @(posedge any_ntf_tch_stream_updated) begin
  if (ntf_tch_stream2_updated) begin
      ntf_tch = ntf_tch_stream2;
  end else if (ntf_tch_stream3_updated) begin
      ntf_tch = ntf_tch_stream3;
  end else if (ntf_tch_stream4_updated) begin
      ntf_tch = ntf_tch_stream4;
  end
end

always @(posedge any_ntf_tah_stream_updated) begin
  if (ntf_tah_stream2_updated) begin
      ntf_tah = ntf_tah_stream2;
  end else if (ntf_tah_stream3_updated) begin
      ntf_tah = ntf_tah_stream3;
  end else if (ntf_tah_stream4_updated) begin
      ntf_tah = ntf_tah_stream4;
  end
end

always @(posedge any_ntf_tdh_stream_updated) begin
  if (ntf_tdh_stream2_updated) begin
      ntf_tdh = ntf_tdh_stream2;
  end else if (ntf_tdh_stream3_updated) begin
      ntf_tdh = ntf_tdh_stream3;
  end else if (ntf_tdh_stream4_updated) begin
      ntf_tdh = ntf_tdh_stream4;
  end
end

always @(posedge any_ntf_twh_stream_updated) begin
  if (ntf_twh_stream2_updated) begin
      ntf_twh = ntf_twh_stream2;
  end else if (ntf_twh_stream3_updated) begin
      ntf_twh = ntf_twh_stream3;
  end else if (ntf_twh_stream4_updated) begin
      ntf_twh = ntf_twh_stream4;
  end
end

always @(posedge any_ntf_twih_stream_updated) begin
  if (ntf_twih_stream2_updated) begin
      ntf_twih = ntf_twih_stream2;
  end else if (ntf_twih_stream3_updated) begin
      ntf_twih = ntf_twih_stream3;
  end else if (ntf_twih_stream4_updated) begin
      ntf_twih = ntf_twih_stream4;
  end
end

always @(posedge clk_dly) begin
  if (mem_stream1_updated) mem_stream1_updated = 1'b0;
  if (qo_reg_stream1_updated) qo_reg_stream1_updated = 1'b0;

  if (write_flag) begin 	//write
    if (no_st_viol) begin 	//write, no viol
      mem_stream1[A] = cd5;
      mem_stream1_updated = 1'b1;
    end
    else begin                 	//write, with viol
      mem_stream1[A] = mem[A] ^ cdx;    //1^x = x
      qo_reg_stream1 = qo_reg ^ cdx;

      mem_stream1_updated = 1'b1;
      qo_reg_stream1_updated = 1'b1;
    end
end //write
  else if (read_flag) begin     //read
    if (no_st_viol) begin 	//read, no viol
      qo_reg_stream1 = mem[marked_a];
      qo_reg_stream1_updated = 1'b1;
    end
    else begin                  //read, with viol
      qo_reg_stream1 = 8'bx;
...
always @(negedge clk_dly) begin         	//invalidate write/read when hold/clk viol
  if (mem_stream2_updated) mem_stream2_updated = 1'b0;
  if (qo_reg_stream2_updated) qo_reg_stream2_updated = 1'b0;
  if (ntf_tch_stream2_updated) ntf_tch_stream2_updated = 1'b0;
  if (ntf_tah_stream2_updated) ntf_tah_stream2_updated = 1'b0;
  if (ntf_tdh_stream2_updated) ntf_tdh_stream2_updated = 1'b0;
  if (ntf_twh_stream2_updated) ntf_twh_stream2_updated = 1'b0;
  if (ntf_twih_stream2_updated) ntf_twih_stream2_updated = 1'b0;

  if (no_hd_viol == 0 | no_ck_viol == 0) begin
    if (write_flag_dly) begin
      if (ntf_twh) begin
        mem_stream2[marked_a]  = mem[marked_a] ^ 8'bx; //GWEN can't be used to generate cdx
        mem_stream2_updated    = 1'b1;
        qo_reg_stream2         = qo_reg ^ 8'bx;
        qo_reg_stream2_updated = 1'b1;
      end
      else begin
        mem_stream2[marked_a]  = mem[marked_a] ^ cdx;
        mem_stream2_updated    = 1'b1;
        qo_reg_stream2         = qo_reg ^ cdx;
        qo_reg_stream2_updated = 1'b1;
      end
    end
    else if (read_flag_dly) begin
    ...
    ntf_tch_stream2_updated = 1'b1;
    ntf_tah_stream2_updated = 1'b1;
    ntf_tdh_stream2_updated = 1'b1;
    ntf_twh_stream2_updated = 1'b1;
    ntf_twih_stream2_updated = 1'b1;
  end
  else begin
    #100;
    ntf_tch_stream2  = 0;
...
always @(posedge cen_fell) begin	//reset fasle notifiers
  if (ntf_tcs_stream3_updated) ntf_tcs_stream3_updated = 1'b0;
  if (ntf_tas_stream3_updated) ntf_tas_stream3_updated = 1'b0;
  if (ntf_tds_stream3_updated) ntf_tds_stream3_updated = 1'b0;
  if (ntf_tws_stream3_updated) ntf_tws_stream3_updated = 1'b0;
  if (ntf_twis_stream3_updated) ntf_twis_stream3_updated = 1'b0;
  if (ntf_tch_stream3_updated) ntf_tch_stream3_updated = 1'b0;
  if (ntf_tah_stream3_updated) ntf_tah_stream3_updated = 1'b0;
  if (ntf_tdh_stream3_updated) ntf_tdh_stream3_updated = 1'b0;
  if (ntf_twh_stream3_updated) ntf_twh_stream3_updated = 1'b0;
  if (ntf_twih_stream3_updated) ntf_twih_stream3_updated = 1'b0;

  ntf_tcs_stream3  = 0;				//after CEN reset (CEN from 1 to 0)
  ntf_tas_stream3  = 0;
  ntf_tds_stream3  = 0;
  ntf_tws_stream3  = 0;
  ntf_twis_stream3 = 0;
  ...
  ntf_tcs_stream3_updated = 1'b1;
  ntf_tas_stream3_updated = 1'b1;
  ntf_tds_stream3_updated = 1'b1;
  ntf_tws_stream3_updated = 1'b1;
  ntf_twis_stream3_updated = 1'b1;
  ntf_tch_stream3_updated = 1'b1;
  ntf_tah_stream3_updated = 1'b1;
  ntf_tdh_stream3_updated = 1'b1;
  ntf_twh_stream3_updated = 1'b1;
  ntf_twih_stream3_updated = 1'b1;
end

always @(negedge clk_dly) begin	//reset setup/hold notifiers
  if (ntf_tcs_stream4_updated) ntf_tcs_stream4_updated = 1'b0;
  if (ntf_tas_stream4_updated) ntf_tas_stream4_updated = 1'b0;
  if (ntf_tds_stream4_updated) ntf_tds_stream4_updated = 1'b0;
  if (ntf_tws_stream4_updated) ntf_tws_stream4_updated = 1'b0;
  if (ntf_twis_stream4_updated) ntf_twis_stream4_updated = 1'b0;
  if (ntf_tch_stream4_updated) ntf_tch_stream4_updated = 1'b0;
  if (ntf_tah_stream4_updated) ntf_tah_stream4_updated = 1'b0;
  if (ntf_tdh_stream4_updated) ntf_tdh_stream4_updated = 1'b0;
  if (ntf_twh_stream4_updated) ntf_twh_stream4_updated = 1'b0;
  if (ntf_twih_stream4_updated) ntf_twih_stream4_updated = 1'b0;
  ...
  ntf_tcs_stream4_updated = 1'b1;
  ntf_tas_stream4_updated = 1'b1;
  ntf_tds_stream4_updated = 1'b1;
  ntf_tws_stream4_updated = 1'b1;
  ntf_twis_stream4_updated = 1'b1;
  ntf_tch_stream4_updated = 1'b1;
  ntf_tah_stream4_updated = 1'b1;
  ntf_tdh_stream4_updated = 1'b1;
  ntf_twh_stream4_updated = 1'b1;
  ntf_twih_stream4_updated = 1'b1;
end
endmodule
