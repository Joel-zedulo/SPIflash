...
// EDIT: Added localparam for that unkown UART value
// UART phase accumulator increment
// baud = 115200
// clk  = 48 MHz
// phase_inc = round(baud * 2^32 / clk) = 32'd10_307_922

localparam UART_PHASE_INC = 22'd4123168; // the original 19,200 baudrate; 20MHz clkfreq increment value
//localparam UART_PHASE_INC = 32'd10_307_922; // 115,200 baudrate; @ 48MHz clkfreq increment value
// EDIT END
...
always @(posedge sys_clk) begin
	...
	{uart_phy_tx_tick, uart_phy_tx_phase} <= UART_PHASE_INC;
	if (uart_phy_tx_enable) begin
		{uart_phy_tx_tick, uart_phy_tx_phase} <= (uart_phy_tx_phase + UART_PHASE_INC);
	end
	...
	if (uart_phy_rx_enable) begin
		{uart_phy_rx_tick, uart_phy_rx_phase} <= (uart_phy_rx_phase + UART_PHASE_INC);
	end
